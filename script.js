...
// --- NEW FUNCTION FOR THE DYNAMIC RESOURCE LIBRARY ---
async function initializeResourceLibrary() {
  ...
  try {
    ...
    links.forEach(link => {
      ...
      listItem.innerHTML = `
        <a href="${link.url}" target="_blank" class="text-xl font-semibold text-fuchsia-400 hover:underline">${link.title}</a>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initializeResourceLibrary(); // The new function is now being called
});