...
- **DRF AV Lab Upgrade:** Integrated Corporate AV Toolkit v2.0.0 into the David Rubenstein Forum portal (`/DRF/drf-media-toolkit.html`, `.css`, `.js`).
  ...
  - Retooled and simplified the "Distance to delay" calculator to take distance in feet and compute millisecond delay based on the standard constant speed of sound (1130 ft/s).
...
### Security
- **CI/CD Hardening:** Upgraded GitHub Actions `github/codeql-action/upload-sarif` from `v3` to `v4` in `security.yml` to resolve deprecation warnings and ensure long-term pipeline security scanning ahead of the December 2026 deprecation.
...
### Changed
- **Personal Copy Polish:** Overhauled copy on the landing page (`index.html`) and toolkit page (`toolbag.html`) to implement a "slop-free" (slopless) writing style. Replaced generic, AI-like buzzwords (e.g. "seamless," "tangible results," "passion") with concrete, professional AV/IT and Field Engineering details (e.g. multicast routing, RF coordination, Dante zones, and packet-level analysis) to present a more authentic professional identity to hiring managers.

## [2.1.1] - 2026-06-15

### Fixed
- **Toolkit Script Reference:** Fixed `ReferenceError: iconKey is not defined` in `js/toolbag.js` by correctly referencing `icons['software']`. This resolved an issue where the page halted rendering after the Hardware section, and restores the Software, Live Production, and Services sections.
